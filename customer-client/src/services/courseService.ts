...
const MENTORS_URL = `${BASE_URL}/api/mentors`;
...
export const getMentors = async (): Promise<IMentor[]> => {
  try {
    console.log("Attempting to fetch mentors from:", MENTORS_URL);
    const headers = await getAuthHeader();
    console.log("Using headers:", headers);
    
    const response = await axios.get(MENTORS_URL, { 
      headers,
      timeout: 10000 // 10 second timeout
    });
...
export const getMentorById = async (id: string): Promise<IMentor> => {
  try {
    const headers = await getAuthHeader();
    const response = await axios.get(`${MENTORS_URL}/${id}`, { headers });
    return response.data.data;
  } catch (error) {
...
export const getMentorCourses = async (mentorId: string): Promise<ICourse[]> => {
  try {
    const headers = await getAuthHeader();
    const response = await axios.get(`${MENTORS_URL}/${mentorId}/courses`, { headers });
    return response.data.data;
  } catch (error) {
